...
% Float, table, and figure formatting for a clean final submission.
...
% Compile safely even if a figure file is missing in the local folder.
% If the image exists in Overleaf, it will render normally.
\newcommand{\safeincludegraphics}[2][]{%
  \IfFileExists{#2}{\includegraphics[#1]{#2}}{%
    \fbox{\parbox{0.90\textwidth}{\centering Missing figure file: \texttt{\detokenize{#2}}}}%
  }%
}
...
I started with a simple question: Does increasing complexity in the model enhance trading results, or does it simply increase the estimation errors and the costs associated with implementing them? We found that models with increased levels of complexity (e.g., additional parameters) place greater demands upon the cost of implementing these models due to the need to provide highly sensitive calibration. Also, complex models like the Kalman that dynamically adjust weights based on future data produce delayed trade signals compared to simpler models since the data is not weighted heavily on the posterior. Therefore, rather than asking if a more complicated model can be fit to past data (i.e., in sample), we want to know if this produces better trade signals once the costs of producing these signals have been subtracted.

To address this question, I test two separate trading methodologies independently. Firstl, a Sector Rotation Strategy: for this strategy, an investment decision is made monthly regarding which of eight U.S. Equities sectors' ETFs should be held. This decision is made by looking at each sector's recent momentum which is the delta in their price within last two lookback windows. There is a much simpler implementation (the base case) but our model intentionaly includes the Bayesian Information Criterion (BIC). The BIC model selected how far back to use in determining momentum for each sector but the base case looks back twelve months for all sectors.
...
In total, the back test ran from January 2016 through December 2024. The years 2015--16 were used as a warm up to determine optimal parameters for both models. The initial capital balance was \$100,000. Equity trades had a transaction fee of 5 bps (\$0.05) per dollar traded. FX trades had a round-trip commission fee of 3.00 pips (\$30.00 USD per pip x 2 sides = \$60.00 USD).

The remainder of this paper will follow the following as follows:

We will look at Data Description and the Methodology for both of the trading methods. Thereafter, Section 4 outlines and displays results for both methodologies for the entire testing period. Section 5 examines results for both methodologies across different subperiods and also across different combinations of parameter values. Finally, we summarize our results, highlight the limitations, and suggests future work.
...
Sector momentum strategies take advantage of the observed phenomenon that industries which have performed well historically will likely do so again in the future during some horizon period. Historically, the most common method of implementing this has been to rank the sectors by their last 12-month performance and then rotate your money into the highest performing sectors on a monthly basis.
...
Where $k$ represents the number of estimated parameters in the AR(1) model, which is 2; the constant term and the lag coefficient, $n$ is the number of observations within the specific window being evaluated, and $\hat{L}$ represents the maximized value of the log-likelihood function. The negative term is higher if the fit is better while the positive term with $n$ adds compelxity, the lower the BIC value the better.
...
In order to construct the portfolio, each of the eight sectors their momemtum was calculated monthly. Then, using these values, each sector was ranked by its momentum return. Each month, the portfolio consisted of the top three sectors in equally weighted allocations, each allocated 33.3\%. Additionally, a long short version of this strategy was tested wherein bottom-ranked sectors with negative momentum were also shorted in addition to the top-ranked sectors with positive momentum.
...
We also calculate the spread. The spread is the difference between the actual returns and what would be expected if the relationships were perfectly linear. Since we want to compare the spread over time, we need to center it around 0. Therefore we subtract the mean of the residuals. Finally, we perform the Augmented Dickey-Fuller test to verify that the spread is stationary before including it in our model. For example, our tests show that our AUD/USD vs. NZD/USD basket has a p-value less than $0.01$ with a hedge ratio of about $0.94$.
...
Here we have defined $\phi$ as the coefficient for mean reversion. It will be estimated using OLS regression $y = \phi x$, where $x = \tilde{\eta}_{t-1}$, and $y = \tilde{\eta}_t$. We bound $\phi$ to $[0.05, 0.99]$ so that the process is both stationary and reasonably useful and it fits the standard for AR parameters to be under 1. On average, we find that for our 24 baskets in the sample, our estimated values of $\phi$ are around $0.96$, i.e., they correspond to an approximate theoretical half life of just under 16 trading days, based on $t_{1/2} = \ln(2) / -\ln(\phi)$.
...
Compared to the 12-month baseline, the added BIC complexity returned an additional 82 basis points of return per annum, a total of 14.12\% versus 13.3\%. Its sharpness also increased to 0.69 compared to 0.62 for the base line, a more significant increase considering the two methods have similar transaction costs. While maximum draw-down was marginally greater for BIC (-30.64\%) compared to the base line (-30.23\%), the differences are negligible. BIC's beta relative to SPY (0.883) was only slightly below that of the base-line (0.90), as expected since BIC sometimes reverses out of positions that do not correlate as well with the broader market.

Over the course of nearly nine years, both methodologies tracked SPY pretty closely. While BIC has not been a better performer than SPY, it does provide an incremental improvement over a simple twelve-month base line.

\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{nav_sector_strategies.png}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{bic_lookback_heatmap.png}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{nav_fx_strategies.png}
\caption{Cumulative NAV for FX strategies and SPY, January 2016--December 2024. FX NAV plotted on right axis for clarity.}
\label{fig:fx_nav}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Sub-Period Performance: Sector Strategies}
...
\begin{figure}[H]
    \centering
    \safeincludegraphics[width=0.95\textwidth]{subperiod_sector.png}
    \caption{Annualized return, Sharpe ratio, and maximum drawdown by sub-period for sector strategies.}
    \label{fig:subperiod_sector}
\end{figure}
...
\begin{figure}[H]
    \centering
    \safeincludegraphics[width=0.95\textwidth]{subperiod_fx.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \safeincludegraphics[width=\textwidth]{robustness_heatmap.png}
        \caption{Sector: Sharpe by lookback $\times$ top-N grid}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \safeincludegraphics[width=\textwidth]{fx_param_sweep.png}
...
Looking at the sector results, the case for adaptive lookback selection is pretty convincing, though it matters a lot which environment you are looking at. The rate-hike period from 2022 to 2024 is probably the best example. Sectors that had been moving together for years started diverging sharply once rates started rising, and a strategy locked into a 12-month lookback was still averaging in all that prior co-movement history. That slowed it down considerably. BIC shifted toward shorter windows naturally during this stretch, picking up the rotation earlier and without needing any manual parameter adjustment.

The way I think about it is that BIC is not necessarily making a smarter call than a fixed lookback on average. Its real advantage comes from not making the worst calls that a static lookback makes when the market shifts suddenly. The parameter grid in Section 5 supports this somewhat: no single fixed window consistently beat BIC across all three sub-periods, which suggests the value does seem to come from the flexibility itself rather than one window just happening to be the better choice overall.

\subsection{When Does Complexity Hurt?}

The FX results go the other direction. The Kalman filter smooths the z-score by treating large single-day moves as partially random rather than fully informative. In many settings that is a useful property. The issue here is that a lot of the FX spread deviations in this sample tended to revert within just a few trading days, and the smoothing adds a lag of roughly two to three days before the filtered z-score reaches the entry threshold. By that point, a meaningful portion of the mean reversion had already played out.

There is a fairly intuitive way to frame when the Kalman filter helps versus when it does not. It is most useful when the observation noise $R$ is large relative to the process noise $Q$, meaning the raw signal is not very reliable on its own. For these FX baskets, the raw spread seems to track the true signal well enough that the extra filtering ends up removing real information rather than cleaning up noise. That is probably the core reason OLS came out ahead, despite being a much simpler approach.

\subsection{Portfolio-Level Implications}

One thing the standalone Sharpe ratios do not capture is how little the FX strategies move with SPY. Correlations of 0.037 and 0.029 are essentially zero. The monthly returns heatmap in Figure~\ref{fig:monthly_heatmap} shows this concretely: March 2020 is a deep red column for SPY and the sector strategies, but the FX rows are close to neutral. A strategy that barely registers during a major equity sell-off has real portfolio value, even if its own annual returns are modest. I do not think these strategies should be dismissed just because their standalone Sharpe ratios happen to be slightly negative.

\begin{figure}[H]
    \centering
    \safeincludegraphics[width=0.95\textwidth]{monthly_returns_all_heatmap.png}
...
There are a few limitations worth keeping in mind when interpreting these results.

\textbf{In-sample candidate set for BIC.} Each month, BIC selects from among three candidate windows (3, 6, and 12 months) using data available up to that point, which is appropriate for a walk-forward test. The issue is that the decision to restrict the search to those three windows was itself made with some knowledge of the literature. A stricter test would separate a design period, used to pick the candidate set, from the evaluation period used here.

\textbf{Data quality.} Yahoo Finance adjusted close prices are a convenient starting point, but they are not institutional-grade data. FX cross rates are constructed from USD legs rather than directly observed, and the 5bp and 1.5-pip transaction cost assumptions are rough approximations. Real spreads and market impact would vary with trade size and how volatile conditions were at the time.

\textbf{No leverage or margin modeling.} The FX strategy implicitly uses leverage since capital is allocated across multiple open baskets at the same time. Margin requirements, overnight financing costs, and forced liquidation risk are not modeled here, and these would all reduce performance in a live account.

\textbf{Regime sensitivity.} The full-period advantage of BIC over the 12-month baseline is largely concentrated in the 2022 to 2024 sub-period. If the sample had ended in 2021, the conclusion might look quite different. The nine-year results support the BIC approach, but the sub-period breakdown shows the outperformance is not consistent across every environment.

%--------------------------------------------------------------
\section{Possible Extensions}
%--------------------------------------------------------------

There are several directions that would be worth pursuing as follow-up work.

\textbf{Walk-forward candidate set validation.} The most straightforward improvement would be to use a held-out period to check whether the 3/6/12-month candidate set actually generalizes out of sample. This would require splitting the historical data into a design period and a separate evaluation period, rather than treating the full backtest as a combined estimate.

\textbf{Regime filter.} Both strategies performed differently depending on the macro environment in ways that were hard to predict ahead of time. Adding an overlay, like pausing momentum positions when VIX exceeds some threshold or conditioning on the yield curve slope, could reduce the strategy's vulnerability to rapid market-wide drawdowns without giving up much return in calmer periods.

\textbf{Intraday FX data.} The core problem with Kalman for this strategy is a mismatch between the filter's natural time scale and how quickly the FX spreads actually revert. Moving from daily to hourly data would shrink the effective lag from two or three days to a few hours. This would need a proper data vendor rather than Yahoo Finance but would give a cleaner test of whether the Kalman filter can add value at a higher frequency.

\textbf{Risk-parity combination.} The near-zero correlation between the FX book and the sector book is basically unused in this analysis since the two are evaluated independently. A risk-parity allocation that weights each strategy by its inverse historical volatility would explicitly take advantage of this diversification property and would likely improve the combined Sharpe ratio by a meaningful amount.

\textbf{Live paper trading.} All of the conclusions here are based on backtested data, which has well-known limitations. The real test would be a forward paper-trading implementation through something like Interactive Brokers' API. That would provide genuine out-of-sample evidence for whether these strategies hold up outside the 2016 to 2024 window.

%--------------------------------------------------------------
\section{Conclusion}
%--------------------------------------------------------------

The main question this project tried to answer was whether adding complexity to a trading model actually produces better results, or whether it mostly adds noise and slows down the signals. The answer turned out differently for the two strategies, which I think is the more interesting result.

For sector momentum, BIC-based adaptive lookback selection did produce a consistent improvement over the fixed 12-month baseline across the nine-year period: 82 basis points more in annualized return and a Sharpe ratio that rose from 0.622 to 0.690. The improvement was clearest during the 2022 to 2024 rate-hike period, when sector leadership rotated quickly enough that being able to shift toward shorter lookbacks made a real difference. That supports the view that adaptive complexity is worth the added overhead when the underlying market is actually shifting between regimes.

For FX arbitrage, the Kalman filter did not outperform the simpler OLS baseline on any full-period metric. The reason is fairly clear in hindsight: the filter introduces a lag that is longer than most of the profitable windows in fast-reverting FX markets. This is a useful negative result in its own right. Smoothing is not automatically helpful, and whether the Kalman filter adds value depends heavily on how quickly the underlying signal moves relative to how long the filter takes to respond.

The broader lesson I take from this is that complexity is only worth the cost when it correctly adapts to the structure of the market it is applied to. A model that picks up genuine regime shifts will outperform a static baseline. A model that smooths out the very signal it is supposed to be tracking will not.
