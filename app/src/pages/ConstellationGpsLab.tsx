/**
 * ConstellationGpsLab — merged knowledge-graph + GPS route explorer.
 * Replaces the production /knowledge-graph page once approved.
 * /constellation-gps-lab (Lab tile on Dashboard)
 *
 * Interactions:
 *   • Click a node → concept detail panel (status, ingredients, mastery)
 *   • Type in search OR click "Plot route" → GPS route panel (prereq chain)
 */
...
export default function ConstellationGpsLab() {
  ...
  const levels = useMemo(() => {
    ...
    for (const n of kgData?.nodes ?? []) if (n.level) s.add(n.level)
