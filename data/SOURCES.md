...
| **OpenStax** — 1st editions, no NC co-tag | `ml/data/external/openstax/free_response_ccby.json` (601 items) | **CC BY 4.0** | ✅ yes | 2026-08-17, CMS API per slug + per-item tag audit | Prealgebra, Elementary/Intermediate Algebra, Statistics. **Attribution required** — carried in `_meta.attribution`. Free-response, **not bank-ready**. |
| **OpenStax** — 1st editions **also tagged to a `-2e`** | not harvested (1,093 items) | **CC BY 4.0 claimed via irrevocability, unconfirmed** | ⚠️ **decision pending** | 2026-08-17 | Same exercise appears in a CC BY 1e and an NC-SA 2e. CC grants are irrevocable, so the 1e grant plausibly survives — but the API serves the *latest* version, which may be 2e-revised text. See spec §S5. |
