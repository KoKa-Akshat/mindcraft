...
import { seedAssessment, recordOutcomes, agentCheckIn, fetchExamConceptIds, type OutcomeInput } from '../lib/mlApi'
...
import { markDiagnosticComplete, markWeeklyPaperComplete, savePracticeDraftRemote, loadPracticeDraftsRemote, loadDiagnostic, getUserRole } from '../lib/practiceState'
...
import { loadCachedWeeklyPaper, type WeeklyPaperSlot, type WeeklyPracticePaper } from '../lib/weeklyPracticePaper'
...
import { sanitizeAnswer, safeSvgHtml, MAX_ANSWER_CHARS } from '../lib/inputGuards'
...
type PracticePhase =
  ...
  | 'no-content'
  /** Weekly Review play-through: a per-slot story/context beat, shown before
   *  that slot's `explore` formula card. See startWeeklyWalkthrough(). */
  | 'weekly-story'
...
export default function Practice() {
  ...
  /** Tier-3 misconception targeting for weakness missions. */
  const [sessionMisconceptionTarget, setSessionMisconceptionTarget] = useState<{
    ingredientId?: string
    misconceptionId?: string
  } | null>(null)
  /** Set when this session was launched from Dashboard's "This week's paper"
   * CTA. If the mission actually finishes (finishSession), the weekKey gets
   * written to users/{uid}.weeklyPaperCompletedWeek so the CTA re-locks
   * until next week. Left null for every other entry point. */
  const [weeklyPaperWeekKey, setWeeklyPaperWeekKey] = useState<string | null>(null)
  /** Weekly Review play-through: set while walking through paper.slots[] in
   *  sequence (story beat → formula card → question batch, repeat, then the
   *  normal `complete` phase). Null for every other entry point — missionType
   *  stays null too, so this never collides with the weakness/learn/gapscan
   *  draft-autosave slots. */
  const [weeklyWalkSlots, setWeeklyWalkSlots] = useState<WeeklyPaperSlot[] | null>(null)
  const [weeklyWalkIndex, setWeeklyWalkIndex] = useState(0)
  const [weeklyWalkQuestionsBySlot, setWeeklyWalkQuestionsBySlot] = useState<Record<string, Question[]>>({})
  ...
  // The solver's own internal state (sPhase, problem, session, …) now lives in
  // <SolverPanel>. This is just the one piece of cross-mode handoff data this
  // component still needs to originate: a pre-filled problem (dashboard
  // quick-box, or a stuck practice question's walkthrough link) that should
  // auto-submit as soon as the panel mounts. See the `state?.problemText`
  // branch below.
  const [solverInitialProblemText, setSolverInitialProblemText] = useState<string | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    const matteOn = mode === 'practice' && ['explore', 'level', 'checkin', 'session', 'complete', 'weekly-story'].includes(pPhase)
  ...
  useEffect(() => {
    const state = location.state as {
      ...
      weeklyPaper?: boolean
      weeklyPaperWeekKey?: string
      weeklyWalkthrough?: boolean
    } | null
    if (state?.problemText) {
      setMode('solver')
      setSolverInitialProblemText(state.problemText)
      ...
      const weeklyPaperFlag = state.weeklyPaper && state.weeklyPaperWeekKey ? state.weeklyPaperWeekKey : null
      void launchMissionDirect(
        state.conceptId,
        state.missionType ?? 'weakness',
        state.formatId,
        state.ingredientId || state.misconceptionId
          ? { ingredientId: state.ingredientId, misconceptionId: state.misconceptionId }
          : undefined,
      ).then(() => {
        // Set AFTER the session actually starts — startSession() clears any
        // stale weeklyPaperWeekKey left over from a previous, abandoned
        // weekly-paper session at the top of its own run, so this can't leak
        // a "done" write onto an unrelated later mission.
        if (weeklyPaperFlag) setWeeklyPaperWeekKey(weeklyPaperFlag)
      })
      window.history.replaceState({}, '')
    } else if (state?.weeklyWalkthrough) {
      const paper = loadCachedWeeklyPaper()
      if (paper) startWeeklyWalkthrough(paper)
  ...
  async function launchMissionDirect(
    ...
    formatId?: FormatId,
    misconceptionTarget?: { ingredientId?: string; misconceptionId?: string },
  ) {
    await _runSession(conceptId, mission, formatId, misconceptionTarget)
  }
  ...
  async function _runSession(
    ...
    setSessionMisconceptionTarget(mission === 'weakness' ? (misconceptionTarget ?? null) : null)
    const diagnostic = await loadDiagnostic(user.uid)
    ...
    await startSession(conceptId, lv, undefined, formatId, misconceptionTarget)
  }
    ...
    try {
      ...
      if (next) {
        void launchMissionDirect(
          ...
          'weakness',
          next.formatId,
          next.misconceptionId || next.ingredientId
            ? { ingredientId: next.ingredientId, misconceptionId: next.misconceptionId }
            : undefined,
        )
  ...
  function resolveSessionLevel(conceptId: string, lv: 1|2|3): 1|2|3 {
    if (questionCount(conceptId, lv) > 0) return lv
    for (const fallback of ([1, 2, 3] as const).filter(l => l !== lv)) {
      if (questionCount(conceptId, fallback) > 0) return fallback
    }
    return lv
  }
  ...
  async function startSession(
    ...
    // Clear any leftover flag from a previous, abandoned "This week's paper"
    // session before this one starts. The weekly-paper launch path (above)
    // re-sets it right after this function resolves, only for its own run.
    setWeeklyPaperWeekKey(null)
    const playableLevel = resolveSessionLevel(conceptId, lv)
    setPPhase('building')
    setConcept(conceptId)
    setLevel(playableLevel)
    setSessionBridge(bridge ?? null)
    const fmt = formatId ?? sessionFormat ?? undefined
    const misTarget = misconceptionTarget ?? sessionMisconceptionTarget ?? undefined

    const examType = (EXAMS.includes(exam as ExamType) ? exam : FALLBACK_EXAM) as ExamType

    evictQuestionCache(conceptId, playableLevel, examType)
    if (missionType) clearPracticeDraft(missionType)

    const staticDraw = misTarget?.ingredientId || misTarget?.misconceptionId
      ? getQuestionsForMisconceptionWeakness(
        conceptId,
        playableLevel,
        SESSION_LENGTH,
      ...
      : getQuestions(conceptId, playableLevel, SESSION_LENGTH, [], examType, fmt, missionType === 'weakness')

    const [dynamic, staticQs] = await Promise.all([
      generateQuestions(conceptId, playableLevel, examType, SESSION_LENGTH, bridge?.fromId),
      Promise.resolve(staticDraw),
    ])
    ...
    if (qs.length === 0) {
      setContentGapMessage(buildNoContentMessage(conceptId, playableLevel, dynamic.length === 0))
  ...
  /** Weekly Review play-through entry point. Groups the paper's already-built
   *  questionIds back into per-slot batches (same grouping pattern
   *  WeeklyPracticePaperPage uses — group by question.conceptId, not by
   *  re-deriving order) and starts the first slot's story beat. Slots whose
   *  concept ended up with zero resolvable questions are dropped rather than
   *  blocking the whole walkthrough. */
  function startWeeklyWalkthrough(paper: WeeklyPracticePaper) {
    const resolved = paper.questionIds
      .map(id => getQuestionById(id))
      .filter((q): q is Question => !!q)
    const bySlot: Record<string, Question[]> = {}
    for (const q of resolved) {
      (bySlot[q.conceptId] ??= []).push(q)
    }
    const activeSlots = paper.slots.filter(slot => (bySlot[slot.conceptId]?.length ?? 0) > 0)
    ...
    setMissionType(null)
    setSessionFormat(null)
    setSessionMisconceptionTarget(null)
    setWeeklyWalkQuestionsBySlot(bySlot)
    setWeeklyWalkSlots(activeSlots)
    setWeeklyWalkIndex(0)
    setWeeklyPaperWeekKey(paper.weekKey)
    setConcept(activeSlots[0].conceptId)
    setPPhase('weekly-story')
  }

  /** Starts the question batch for the walkthrough's CURRENT slot — called
   *  from the reused `explore` formula card's "Start" button once the student
   *  has seen that slot's story beat + formula card. Mirrors startSession()'s
   *  state reset but skips the level picker / dynamic generation: the paper
   *  already picked the level + questions for this slot. */
  function beginWeeklySlotSession(slot: WeeklyPaperSlot) {
    const qs = weeklyWalkQuestionsBySlot[slot.conceptId] ?? []
    if (qs.length === 0) return // pre-filtered in startWeeklyWalkthrough; defensive only
    setConcept(slot.conceptId)
    setLevel((qs[0]?.level as 1 | 2 | 3) ?? 1)
    setSessionBridge(null)
    prefetchStoryModule(slot.conceptId, qs, 0)
    setQuestions(qs)
    setQIndex(0)
    setSelected(null)
    setTypedAnswer('')
  ...
  async function finishSession(
    ...
    // concept's story beat instead of ending the whole walkthrough here.
    if (weeklyWalkSlots && weeklyWalkIndex + 1 < weeklyWalkSlots.length) {
      const nextIndex = weeklyWalkIndex + 1
      setWeeklyWalkIndex(nextIndex)
      setConcept(weeklyWalkSlots[nextIndex].conceptId)
      setPPhase('weekly-story')
      return
    }
    if (weeklyWalkSlots) {
      setWeeklyWalkSlots(null)
      setWeeklyWalkIndex(0)
      setWeeklyWalkQuestionsBySlot({})
    }

    if (weeklyPaperWeekKey) {
      void markWeeklyPaperComplete(user.uid, weeklyPaperWeekKey)
      setWeeklyPaperWeekKey(null)
    }
    setPPhase('complete')
  }
  ...
  const isMatteFlow = mode === 'practice' && ['explore', 'level', 'checkin', 'session', 'complete', 'no-content', 'weekly-story'].includes(pPhase)
    ...
    <div className={`${s.shell}${isPathView ? ` ${s.pathShell}` : ''}${isMatteFlow ? ` ${s.matteShell}` : ''}${isPaperScan ? ` ${s.paperScan}` : ''}`}>
      ...
      <main className={`${s.page}${isPathView ? ` ${s.pathPage}` : ''}${isLessonPage ? ` ${s.lessonPage}` : ''}${mode === 'solver' ? ` ${s.solverPage}` : ''}`}>
        ...
        {mode === 'practice' && (
          <>
            ...
            {pPhase === 'weekly-story' && (conceptMeta || (weeklyWalkSlots && concept)) && (() => {
              ...
              return (
                <div className={s.exploreScreen}>
                  <button className={s.backLink} onClick={() => navigate('/dashboard', { replace: true })}>
                    ← Back to journal
                  </button>
                  ...
                  <div className={s.exploreCard}>
                    ...
                    <button className={s.startPracticeBtn} onClick={() => setPPhase('explore')}>
                      Continue →
                    </button>
                  </div>
                </div>
            ...
            {/* ── Explore: concept content card ──
                Also doubles as the Weekly Review walkthrough's per-slot formula
                card (weeklyWalkSlots set) — walkthrough concept ids aren't all
                in PRACTICE_CONCEPTS (e.g. fractions_decimals, ratios_proportions),
                so id/label fall back to `concept` + actConceptLabel() rather than
                requiring a conceptMeta hit. */}
            {pPhase === 'explore' && (conceptMeta || (weeklyWalkSlots && concept)) && (() => {
              const explId = conceptMeta?.id ?? concept ?? ''
              const explLabel = conceptMeta?.label ?? actConceptLabel(explId)
              const content = getConceptContent(explId)
              return (
                <div className={s.exploreScreen}>
                  <button className={s.backLink} onClick={() => navigate('/dashboard', { replace: true })}>
                    ← Back to journal
                  ...
                  <div className={s.exploreCard}>
                    ...
                    <div className={s.exploreHead}>
                      <span className={s.exploreIconWrap}>
                        <ConceptPathIcon conceptId={explId} size={44} />
                      </span>
                      <div>
                        <h2 className={s.exploreName}>{explLabel}</h2>
                        ...
                        {confidenceMap[explId] && (
                          <span className={s.confLevelHint}>
                            Craft recommends: Level {getRecommendedLevel(explId)} based on your self-assessment
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    {content ? (
                      ...
                      // No curated CONCEPT_CONTENT for this concept yet — skip the
                      // formula card body gracefully rather than blocking the
                      // walkthrough; the student still gets straight to the
                      // question batch for this slot.
                      <p className={s.exploreTagline}>
                        No formula sheet yet for this topic — heading straight to the questions.
                      </p>
                    ) : null}
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (weeklyWalkSlots) beginWeeklySlotSession(weeklyWalkSlots[weeklyWalkIndex])
                        else setPPhase('level')
                      }}
                    >
                      {weeklyWalkSlots ? 'Start these questions →' : 'Start Practice →'}
                    </button>
                  </div>
                </div>
              )
            ...
            {pPhase === 'no-content' && contentGapMessage && (
              <div className={s.levelScreen}>
                <button type="button" className={s.backLink} onClick={() => navigate('/dashboard', { replace: true })}>
                  ← Back to journal
                </button>
                <div className={s.levelHeader}>
                  <div>
                    <h2 className={s.levelConceptName}>More questions coming soon</h2>
                    <p className={s.levelConceptSub}>We do not have enough at this level yet. Head back to your journal and try another topic.</p>
                  </div>
                </div>
                ...
                <div className={s.sessionMain}>
                  <div className={s.progressStrip}>
                    ...
                    <div className={s.stripLeft}>
                      ...
                      {weeklyWalkSlots && (
                        <span className={s.stripBridge}>
                          Weekly Review · Topic {weeklyWalkIndex + 1}/{weeklyWalkSlots.length}
                        </span>
                      )}
                  ...
                  <div className={s.questionCard}>
                    <div className={s.questionBanner} style={{ background: lvBannerGradient }}>
                      ...
                      <p className={s.questionText}><MathText text={currentQ.question} questionId={currentQ.id} graphAlreadyShown={!!graphPoints || !!graphExpr} /></p>
                    </div>
        ...
        {mode === 'solver' && (
          <SolverPanel
            exam={exam}
            initialProblemText={solverInitialProblemText}
