...
3. Keep any LaTeX (\\( \\), $ $, \\[ \\]) exactly as written. Every expression must stay in LaTeX so the app renders it. Never convert an expression to plain prose digits, and never rewrite it into an equivalent form: \\((-9)+(-4)\\) must NOT become "-9 - 4". The operator, the order, and the grouping are part of what the question tests.
4. FORMAT: a SHORT WORD PROBLEM. One or two sentences of setup, then the mathematical ask. Hard ceiling of 45 words before the ask. Every sentence must carry either a number's meaning or the ask itself. Do NOT add sentences that only restate why the math matters ("The calculation is necessary to inform the shipper." "An accurate estimate avoids confusion."). If you have said what each quantity IS and what is being asked, STOP. Each storyStem must stand alone: do NOT reference other questions or "the previous scene", since questions can appear in any order.
5. GIVE EVERY NUMBER A REFERENT. The word problem must say what each quantity actually IS inside the story: not "Ramanujan combines two ledger debts, evaluate \\((-9)+(-4)\\)", but "Ramanujan owes 9 rupees for apples and 4 for bananas. Written as debts, \\((-9)+(-4)=\\)". The referents must make the SIGN and the OPERATION feel inevitable: a debt is why a number is negative, combining two debts is why they add. If a number cannot be given an honest referent, leave the expression as the thing the character is reading and do not invent a fake one.
6. Use the story's actual characters, places, and stakes. If the original stem's context conflicts with the story world, translate the surface context but keep the math identical. The math must be WOVEN INTO the scene's action: a named character needs this exact quantity for a concrete reason. Never a scene followed by an unrelated textbook ask.
7. BAD (padding, no referents): "Srinivasa Ramanujan is tasked with estimating a value to determine the total cost of a shipment at the Madras port office in 1913. The calculation is necessary to inform the shipper of the expected costs. Ramanujan must provide an accurate estimate to avoid confusion." GOOD (short, every number means something): "Ramanujan owes 9 rupees for apples and 4 for bananas. Written as debts, what is \\((-9)+(-4)=\\)?"
