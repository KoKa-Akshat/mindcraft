#!/usr/bin/env python3
"""Generate a PDF of RESEARCH_HANDOFF_SUMMARY.md, the condensed bullet-form
digest of the Research Constitution, using the same house style as
generate_research_constitution.py and docs/canon/generate_brand_book.py."""
...
MD = ROOT / "RESEARCH_HANDOFF_SUMMARY.md"
OUT = ROOT / "RESEARCH_HANDOFF_SUMMARY.pdf"
...
def add_page(canvas, doc):
    ...
    canvas.drawCentredString(
        ...
        f"MindCraft Research Handoff Summary  ·  page {doc.page}",
    )
    canvas.restoreState()
...
def main():
    ...
    doc = SimpleDocTemplate(
        ...
        title="MindCraft Research Handoff Summary",
    ...
    cover = [
        ...
        Paragraph("Research Handoff Summary", styles["cover_title"]),
        Spacer(1, 0.3 * inch),
        Paragraph(
            "A product-focused digest of the Research Constitution: competitive landscape, "
            "pricing, metrics, and the SAFE-* product rules that survived Red Team.",
            styles["cover_sub"],
        ),
        Spacer(1, 0.5 * inch),
        HRFlowable(width="40%", thickness=1, color=ACCENT, spaceBefore=8, spaceAfter=12),
        Paragraph(
            f"Source: 160 chapters, 2026-07-25 to 2026-08-17, ~{word_count:,} words in this digest.",
            styles["meta"],
        ),
        Spacer(1, 1.2 * inch),
        PageBreak(),
