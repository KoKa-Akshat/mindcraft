...
import { selectPublishable, driveMode } from '../src/drive.mjs'
...
test('drive mode picks oauth first and refuses with no credentials',()=>{assert.equal(driveMode({MARKETING_DRIVE_REFRESH_TOKEN:'r',MARKETING_DRIVE_SERVICE_ACCOUNT:'s'}),'oauth');assert.equal(driveMode({MARKETING_DRIVE_SERVICE_ACCOUNT:'s'}),'service_account');assert.equal(driveMode({}),null)})
