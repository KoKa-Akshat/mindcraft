...
Queued next: see `NEXT_LAB.md` (Part CLV gold-set path×stratum probes, then 156+). Company law through CL (v1.19); CLI–CLIV provisional pending synthesizer.
...
**Next research bottleneck:** Part CLV **Gold-set planted probes for path×stratum monitors**. Company law through SAFE-SURGEPANEL (CL); **SAFE-REVOKE (CLI) + SAFE-PATHBLAME (CLII) + SAFE-PANELRAMP (CLIII) + SAFE-DEMOB (CLIV) provisional** densifiers pending synthesizer. Experiment families …/SURGEPANEL/**REVOKE**/**PATHBLAME**/**PANELRAMP**/**DEMOB** gate claims above L1. Researcher count since v1.19 synthesizer = **4**.
...
3cs. Gold-set planted probes for path×stratum monitors — **NEXT** id 155  
3ct. Generator vs rater poison separation on inherit repair — queued id 156  
...
134. **NEXT:** Gold-set planted probes for path×stratum monitors (id 155)
