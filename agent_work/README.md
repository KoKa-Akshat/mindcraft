...
| [engine/ITEM_PREDICTOR_BUILD.md](engine/ITEM_PREDICTOR_BUILD.md) | Engine | Open — decouple the predictor from mastery. Today the harness uses concept mastery *as* P(correct), ignoring the question entirely; adds an IRT-style `σ(a·(θ−b))` in `validation/` with difficulty read from ontology priors. Gives issue #15 its first consumer. Prereq: dedup observations |
| [cross-cutting/PRACTICE_LOOP_EVIDENCE_BUILD.md](cross-cutting/PRACTICE_LOOP_EVIDENCE_BUILD.md) | Product (A, C) + Engine (B) | Open — **ordered**: idempotent finish → repeat-aware `exposure_weight` → 3-lives requeue. C multiplies same-question repeats by design, so it is only safe after B. Fixes the 28%/46% duplicate-evidence bug |
| [engine/INGREDIENT_MASTERY_REWIRE_BUILD.md](engine/INGREDIENT_MASTERY_REWIRE_BUILD.md) | Engine | Open — misconceptions move ingredients but ingredients move nothing; `aggregate_to_concept_mastery` exists uncalled. Must emit a low-weight `SessionEvent` (not overwrite `mastery_by_concept`) or the next rebuild discards it. Adds misconception multiset substrate |
| [engine/CLASSIFIER_INGREDIENT_AUDIT_PROMPT.md](engine/CLASSIFIER_INGREDIENT_AUDIT_PROMPT.md) | Engine | Open — **audit prompt, not a spec.** Assess classifier + ingredient enrichment + generation as one system, and test whether fully-annotated generation closes the ingredient-tag / 82-ingredient / null-distractor / zero-coverage gaps at once. Deliverable: `CLASSIFIER_INGREDIENT_AUDIT.md` |
