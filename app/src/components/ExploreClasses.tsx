...
const CARD_W  = 118 + 12  // card width + gap
const VISIBLE = 4
...
export default function ExploreClasses() {
  ...
  const offset  = page * VISIBLE * CARD_W
    ...
    <div className={s.wrap}>
      <div className={s.header}>
        ...
        <div className={s.arrows}>
          <button className={s.arrow} onClick={() => setPage(p => Math.max(p - 1, 0))} disabled={page === 0} aria-label="Previous">‹</button>
          <button className={s.arrow} onClick={() => setPage(p => Math.min(p + 1, maxPage))} disabled={page >= maxPage} aria-label="Next">›</button>
        </div>
      </div>

      <div className={s.viewport}>
        <div className={s.scrollRow} style={{ transform: `translateX(-${offset}px)` }}>
          {loading
            ? [1,2,3,4].map(i => <div key={i} className={`${s.card} ${s.skeleton}`} />)
            : cards.map((c, i) => (
